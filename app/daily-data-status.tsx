...
type Status={state:string;startedAt?:string;completedAt?:string;expectedSessions?:{TW:string;US:string};coverage?:{TW:{total:number;ready:number};US:{total:number;ready:number}};latestAttempt?:{state:string;error?:string}};
export function DailyDataStatus() {
  const [status,setStatus]=useState<Status|null>(null);
  useEffect(()=>{
    const load=()=>fetch('/api/daily-status',{cache:'no-store'}).then(r=>r.json()).then(setStatus).catch(()=>setStatus({state:'unavailable'}));
    void load();const timer=setInterval(()=>void load(),60000);return()=>clearInterval(timer);
  },[]);
