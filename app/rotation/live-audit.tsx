...
export default function LiveAudit(){
  ...
  return <section className={styles.panel} aria-labelledby="live-audit-heading">
    ...
    {data&&<>
      ...
      {data.compared.some(r=>r.valuationReviewRequired)&&<div className={styles.notice}>
        <strong>{t('待覆核估值：僅供差異研究，不列入估值推薦排名','Review-required values: comparison research only, excluded from valuation rankings')}</strong>
        <p>{data.compared.filter(r=>r.valuationReviewRequired).map(r=>`${r.ticker} ${r.name}`).join(' · ')}</p>
      </div>}
      <div className={styles.tableWrap}><table className={styles.table}><caption>{t('逐股比較：依你提供的外部上漲空間排序','Stock comparison, ordered by your reference upside')}</caption><thead><tr>{[t('標的','Stock'),t('參考／本站股價','Reference / site price'),t('外部公允價值','Reference FV'),t('本站公允價值','Site FV'),t('差異','Gap'),t('本站模型數','Site models'),t('資料狀態','Data status')].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{data.compared.map(r=><tr key={r.ticker}><td><a href={stockDetailHref(r.ticker)}>{r.ticker} {r.name}</a></td><td>{num(r.price)} / {num(r.observation?.price)}</td><td><a href={r.source} target="_blank" rel="noreferrer">{num(r.fairValue)}</a><small className={styles.cellNote}>+{num(r.upsidePct,1)}%</small></td><td>{num(r.actual?.fairValue)}</td><td>{signed(r.gapPct)}{r.gapPct===null?'':'%'}</td><td>{r.observation?.modelCount??'—'}</td><td>{r.valuationReviewRequired?t('待覆核，僅供差異研究','Review required; comparison only'):r.aligned?t('同日同價','Aligned'):r.actual?t('日期／價格不同，排除平均','Unaligned; excluded'):t('無可比較估值','No comparable valuation')}<small className={styles.cellNote}>{r.observation?.financialDate}</small><small className={styles.cellNote}>{[...new Set(r.issues)].join(' · ')}</small></td></tr>)}</tbody></table></div>
