...
const SITE_REVISION = "Rev. 2026.08.25.1";

const REVISION_ENTRIES = [
  ["2026.08.25.1", "公允價值排行榜估值差距即時聯動升級：1. 排行榜全數候選標的預先融合官方完整財報數據（4 年 EPS 歷史、自由現金流、負債比等），消除簡化比率與完整多模型估值之差距（如 9945 潤泰新校準至真實多模型公允價值 $39.27）；2. 前端排行榜全面實現即時響應式狀態聯動，點選任一標的或點擊「資料更新」後，排行榜估值差距與詳細頁面 100% 同步即時更新。", "Fair-value ranking valuation gap real-time synchronization upgraded: 1. All ranking candidates pre-enriched with official financial statement history (4-year EPS trends, FCF, debt ratios) eliminating discrepancies between crude market-ratios and multi-model intrinsic values (e.g. 9945 aligned to true multi-model consensus $39.27); 2. Frontend ranking table fully synchronized to live reactive inputs, instantly updating valuation gaps across both the ranking table and detail cards upon selecting or refreshing any ticker."],
  ["2026.08.24.1", "順勢交易（重點時機區）四階段量化法則精準固化：1. 下跌築底（一段連續下跌波段並在底部震盪築底，可配合早晨之星做首次左側佈局）；2. 反轉向上（強勢波段推升，帶動 15EMA 與 50SMA 黃金交叉並向上打開）；3. 平整方框箱體整理（整理區域極為平整，高度 $\le 12.5\%$，箱體內絕無突兀暴衝之假突破，確保籌碼乾淨沉澱）；4. 第 3 次或第 4 次回踩箱體下沿支撐（於 50MA 水平支撐處，15EMA 與 50SMA 完全收攏黏合，為唯一重點時機重裝起漲買點）。", "Trend Pullback (High-Edge Buy Zone) 4-stage quant rules rigorously codified: 1. Prior downtrend & base bottoming (sustained decline finding base support, open to initial Morning Star layout); 2. Impulse surge (strong upward thrust driving EMA15/SMA50 golden cross and widening MA spread); 3. Flat box consolidation (tight horizontal range <= 12.5% without erratic false breakout spikes to ensure clean supply absorption); 4. 3rd or 4th retest of lower support floor (precise test of 50MA horizontal floor with tight EMA15/SMA50 MA convergence as the prime high-win-rate trend launch entry)."],
  ["2026.08.23.8", "順勢交易（W底買點）規則全面嚴格化：1. 均線強制要求金叉（15EMA 必須在 50SMA 之上，死叉標的一律剔除）；2. 盤整回調時，必須已完成『第二次（或第三次）回踩前次支撐低點』且兩均線收攏時才列入觀察（排除如 8299 群聯等均線死叉且尚未回踩前次低點之標的）；3. 校準週月關鍵支撐判定，精確識別 8299 於 2026-07-30 在週線支撐 1460 形成的歷史早晨之星。", "Trend Pullback rules strictly upgraded: 1. Strict Golden Cross requirement (15EMA MUST be >= 50SMA, death cross strictly rejected); 2. Price MUST have completed its 2nd (or 3rd) retest of the prior horizontal consolidation low with MA convergence before qualifying (eliminating tickers like 8299 in death-cross without 2nd retest); 3. Calibrated weekly/monthly key levels, accurately recognizing 8299's historical Morning Star candidate at 1460 weekly support on 2026-07-30."],
