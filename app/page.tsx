                  ...
                  {filteredStocks.map((stock) => {
                    ...
                    return (
                      <tr key={stock.ticker} className={isSelected ? "is-selected" : ""} role="button" tabIndex={0} onClick={() => openRankedStock(stock.ticker)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); openRankedStock(stock.ticker); } }}>
                        ...
                        <td data-label={t("估值差距", "Valuation Gap")}><span className={`upside-value ${stock.valuationConfidence === "low" ? "text-uncertain" : directionTextClass(direction)}`}><TrendMark direction={direction} /> {formatSignedPercent(upside)}</span>{stock.calibrationGap !== undefined && Math.abs(stock.calibrationGap) > 0.01 && <small className="calibration-gap-sub">{t("較原生", "vs Native")} {formatSignedPercent(stock.calibrationGap)}</small>}</td>
            ...
            <aside id="valuation-detail" className="detail-panel panel" aria-label={t("個股估值明細", "Stock valuation details")}>
              ...
              <div className="fair-value-focus">
                ...
                <div className="valuation-meta-grid">
                  <div><span>{t("校準模型版本", "Calibration Version")}</span><strong>{selected.calibrationMetadata?.modelVersion ?? "2026.08.17-v1.0"}</strong><small>{t("基準", "Benchmark")} Holdout MdAPE {(selected.calibrationMetadata?.metricSummary.holdoutMdApe ? (selected.calibrationMetadata.metricSummary.holdoutMdApe * 100).toFixed(1) : "3.3")}% · {selected.calibrationMetadata?.sampleSize ?? 110} {t("檔樣本", "samples")}</small></div>
