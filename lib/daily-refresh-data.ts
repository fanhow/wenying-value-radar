...
const SERIES = ['DilutedEPS','TotalRevenue','OperatingCashFlow','CapitalExpenditure','TotalAssets',
  ...
  'OperatingIncome','CashCashEquivalentsAndShortTermInvestments','TotalDebt','TaxProvision','PretaxIncome'];
const DAY = 86400000;
...
export function completedCandles(payload: YahooChartPayload, market: 'TW' | 'US', now = new Date()) {
  ...
  return parseYahooDailyCandles(payload, 400).filter(c => c.date < date || (c.date === date && afterClose));
}
export function quarterlyInputs(payload: SeriesPayload, currency: string, now = new Date()) {
  ...
  const dates = [...(values.get('quarterlyTotalRevenue')?.keys() ?? [])].sort().slice(-4);
  if (dates.length !== 4) throw new Error('FOUR_QUARTERS_UNAVAILABLE');
  for (let i=1;i<4;i++) {
    const gap = (Date.parse(dates[i])-Date.parse(dates[i-1]))/DAY;
    if (gap < 70 || gap > 110) throw new Error('NON_CONTIGUOUS_QUARTERS');
  }
  const end = dates[3], age = (now.getTime()-Date.parse(end))/DAY;
  ...
  const priorTtm = previous('trailingTotalRevenue');
  ...
  const result: Partial<StockInput> = { eps:annualEps, bvps, fcfPerShare:(ocf-Math.abs(capex))/shares,
    ...
    ...valuationTargets(revenueGrowth,roe,debtRatio), sourceNote:`Yahoo Finance TTM（供應商 TTM 或完整連續四季）；營收成長：${growthBasis}` };
  const optional: Array<[keyof StockInput, number | undefined]> = [
    ['ebitPerShare',sum('OperatingIncome')],['cashPerShare',last('CashCashEquivalentsAndShortTermInvestments')],['debtPerShare',last('TotalDebt')],
  ];
...
export async function fetchRefreshRecord(target: RefreshTarget, expectedDate: string, now = new Date(), fetcher: typeof fetch = fetch): Promise<RefreshRecord> {
  ...
  const financialUrl=`https://query2.finance.yahoo.com/ws/fundamentals-timeseries/v1/finance/timeseries/${encodeURIComponent(symbol)}?type=${[...SERIES.map(s=>'quarterly'+s),...['DilutedEPS','TotalRevenue','OperatingCashFlow','CapitalExpenditure','OperatingIncome','NetIncome'].map(s=>'trailing'+s)].join(',')}&period1=${Math.floor(now.getTime()/1000)-DAY/1000*1000}&period2=${Math.floor(now.getTime()/1000)}`;
  ...
  try {
    ...
    const candles=completedCandles(chart,target.market,now), latest=candles.at(-1);
...
export async function expectedSession(market:'TW'|'US',now=new Date(),fetcher:typeof fetch=fetch) {
  ...
  const [p,calendar]=await Promise.all([json(url,fetcher),officialSession(market,now,fetcher)]), candles=completedCandles(p,market,now), date=candles.at(-1)?.date;
