...
export async function handleRefreshWrite(request:Request,db:D1Database|undefined,secret:string|undefined):Promise<Response> {
  ...
  try {
    const raw=await request.text();if(raw.length>1500000)return response({error:'PAYLOAD_TOO_LARGE'},413);
    ...
    if(body.action==='fail') {
      if(run.state==='running')await db.prepare("UPDATE daily_refresh_runs SET state='failed',error=?,completed_at=? WHERE id=?").bind(String(body.error??'COLLECTOR_FAILED').slice(0,250),new Date().toISOString(),id).run();
      return response({state:run.state==='running'?'failed':run.state});
    }
    if(run.state!=='running')return response({state:run.state,runId:id});
    ...
    if(body.action==='finalize') {
      ...
      await db.batch([
        db.prepare('UPDATE daily_refresh_runs SET state=?,summary=?,completed_at=? WHERE id=?').bind(state,JSON.stringify(summary),new Date().toISOString(),id),
        db.prepare('INSERT INTO daily_refresh_head(id,run_id) VALUES(1,?) ON CONFLICT(id) DO UPDATE SET run_id=excluded.run_id').bind(id),
      ]);
      // Retain the active and immediately previous generation, not unbounded OHLC history.
      await db.prepare('DELETE FROM daily_refresh_records WHERE run_id NOT IN (SELECT id FROM daily_refresh_runs ORDER BY started_at DESC LIMIT 2)').run();
