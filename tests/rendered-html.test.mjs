...
test("renders development preview metadata", async () => {
  ...
  assert.match(html, /Rev\. 2026\.09\.01\.8/);
  ...
  assert.match(styles, /\.earnings-panel \{/);
  assert.match(styles, /\.earnings-alert-banner\.imminent \{/);
  ...
  assert.match(source, /valuationDirectionSymbol\(direction\)[\s\S]*formatSignedPercent\(upside\)/);
...
assert.match(fundsSource, /FILING SCHEDULE \/ 02/);
assert.match(fundsSource, /5 月 15 日 前後/);
assert.match(fundsSource, /8 月 14 日 前後/);
assert.match(fundsSource, /11 月 14 日 前後/);
assert.match(fundsSource, /2 月 14 日 前後/);
assert.match(fundsSource, /時間滯後提醒/);
assert.match(fundsSource, /大戶追蹤實戰致勝 3 步驟/);
