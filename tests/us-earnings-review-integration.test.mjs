...
function history(stock){return {ticker:stock.ticker,market:'US',name:stock.name,quoteSource:base.priceSource,candles,weeklyCandles:[],monthlyCandles:[],technicalAnalysis:{...analyzeTechnicalSetup(candles,null),candlestickPattern:'morning-star',patternStage:'confirmed'}};}
function database(seed=false){
  ...
  if(seed){
    ...
    for(const ticker of ['VISN',...Array.from({length:10},(_,i)=>`T${i}`)]){
      const stock={...base,ticker};
      raw.prepare('INSERT INTO daily_refresh_records(run_id,market,ticker,status,stock,history,issues,upside,signals,eligible) VALUES(?,?,?,?,?,?,?,?,?,?)').run(runId,'US',ticker,'ready',JSON.stringify(stock),JSON.stringify(history(stock)),'[]',ticker==='VISN'?30:.2,1,1);
    }
  }
  return db;
}
...
test('review disposition withholds calculated values without mutating raw financials',()=>{
  ...
  assert.deepEqual(state.issues,['US_EARNINGS_BASIS_REVIEW_REQUIRED']);assert.equal(state.review.issuerCik,'1517228');assert.equal(JSON.stringify(input),before);
  assert.ok(dailyValuationState({...base,ticker:'CONTROL'},runId).stock);
});
test('legacy US automatic inputs cannot revive reviewed values; manual and archives remain intact',()=>{
...
test('earnings review is independent of the valuation engine in the four-factor screen',()=>{
...
test('read-time review blocks old stored upside and API values while preserving OHLC and pure technicals',async()=>{
  const db=database(true);
  try{
    const valuation=await read(db,'/api/valuation',{ticker:'VISN',market:'US'});assert.equal(valuation.status,422);
    const v=await valuation.json();assert.deepEqual(v.issues,['US_EARNINGS_BASIS_REVIEW_REQUIRED']);assert.ok(v.earningsReview.sourceUrls.length);assert.equal(v.stock,undefined);
    const historyResponse=await read(db,'/api/price-history?ticker=VISN&market=US');assert.equal(historyResponse.status,200);
    const h=await historyResponse.json();assert.deepEqual(h.candles,candles);assert.equal(h.valuationAvailable,false);assert.equal(h.technicalAnalysis.valueTrendResonance,null);assert.match(h.name,/Vistance/);
    const scan=await (await read(db,'/api/market-scan')).json();assert.equal(scan.candidates.length,10);assert.ok(!scan.candidates.some(s=>s.ticker==='VISN'));
    const tech=await (await read(db,'/api/technical-scan')).json();const signal=tech.morningStar.find(s=>s.ticker==='VISN');assert.ok(signal);assert.equal(signal.upside,null);assert.equal(signal.fairValue,null);assert.ok(!tech.valueTrend.some(s=>s.ticker==='VISN'));
    const rotation=await rotationAudit(db,runId,'US');assert.equal(rotation.top10.length,10);assert.equal(rotation.screen.eligible,10);assert.ok(!rotation.top10.some(s=>s.ticker==='VISN'));assert.ok(!rotation.screen.candidates.some(s=>s.ticker==='VISN'));
    const stored=db.raw.prepare('SELECT stock,upside,eligible FROM daily_refresh_records WHERE ticker=?').get('VISN');assert.equal(JSON.parse(stored.stock).eps,10);assert.equal(stored.upside,30);assert.equal(stored.eligible,1);
    assert.match(scan.freshness.usEarningsReviewVersion,/us-earnings-review/);assert.equal(scan.freshness.usEarningsReviewCoverage,'source-reviewed-cases-only');
  }finally{db.raw.close();}
});
test('new daily writes persist raw ready inputs and liquidity but no reviewed upside',async()=>{
  const db=database(),secret='fixture-only-us-review-000000000000000000';
  const targets=Array.from({length:20},(_,i)=>({ticker:i===0?'VISN':`C${i}`,market:'US'}));
  ...
  try{
    ...
    const row=db.raw.prepare('SELECT * FROM daily_refresh_records WHERE ticker=?').get('VISN');assert.equal(row.status,'ready');assert.equal(row.eligible,1);assert.equal(row.upside,null);assert.equal(JSON.parse(row.stock).eps,10);assert.equal(JSON.parse(row.history).candles.length,80);assert.deepEqual(JSON.parse(row.issues),['US_EARNINGS_BASIS_REVIEW_REQUIRED']);
  }finally{db.raw.close();}
});
